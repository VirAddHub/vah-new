...
export function PDFViewerModal({ 
  ...
  useEffect(() => {
    ...
    async function loadBlob(id: string) {
      ...
      try {
        const resp = await fetch(`/api/mail-items/${id}/download?disposition=inline`, {
          credentials: "include"
        });
  ...
  const handleDownload = () => {
    ...
    const downloadUrl = `/api/mail-items/${mailItemId}/download?disposition=attachment`;
        ...
        <div className="flex-1 bg-muted/20 p-4 overflow-hidden">
          {useBlobFallback || iframeError ? (
            ...
            <iframe
              ...
              src={
                mailItemId
                  ? `/api/mail-items/${mailItemId}/download?disposition=inline#toolbar=1`
                  : 'about:blank'
              }
              className="w-full h-full border-0 rounded bg-white"
              title="Mail Scan Preview"
