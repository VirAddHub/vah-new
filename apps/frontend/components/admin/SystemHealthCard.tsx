...
export default function SystemHealthCard() {
    ...
    if (isLoading) {
        return <div className="rounded-2xl p-4 bg-card animate-pulse h-40" />;
    }

    if (error || !data) {
        return (
            <div className="rounded-2xl p-4 bg-card border border-red-200">
            ...
            <ul className="mt-3 space-y-2">
                ...
                {(deps.dependencies || []).map((d: any) => (
                    <li key={d.name} className="flex items-center justify-between py-1">
                        <span className="text-body-sm capitalize">{d.name.replace(/_/g, ' ')}</span>
