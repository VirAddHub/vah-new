...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!email) {
      console.warn("[reset-password-request] missing email");
    }
    ...
    const res = await fetch(buildUrl("/api/profile/reset-password-request"), {
      method: "POST",
      ...
      console.error("[reset-password-request] fetch failed:", e?.message || e);
      return null;
    });
    ...
    if (!res) {
      ...
      console.error("[reset-password-request] backend unreachable");
      return NextResponse.json(
        { ok: true, message: "If that email exists, we've sent a link." },
        { status: 200 }
      );
    }
    ...
    if (!res.ok) {
      ...
      console.error("[reset-password-request] backend non-OK:", res.status, txt?.slice(0, 200));
      return NextResponse.json(
        { ok: true, message: "If that email exists, we've sent a link." },
        { status: 200 }
      );
    ...
    console.error("[reset-password-request] route error:", e?.message || e, { email });
    return NextResponse.json(
      { ok: true, message: "If that email exists, we've sent a link." },
      { status: 200 }
    );
