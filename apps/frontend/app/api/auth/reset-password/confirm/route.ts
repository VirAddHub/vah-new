...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (!token || !password) {
            console.warn("[reset-password-confirm] missing token or password");
        }
        ...
        const res = await fetch(buildUrl("/api/auth/reset-password/confirm"), {
            ...
            console.error("[reset-password-confirm] fetch failed:", e?.message || e);
            return null;
        });
        ...
        if (!res) {
            ...
            console.error("[reset-password-confirm] backend unreachable");
        ...
        if (!res.ok) {
            ...
            console.error("[reset-password-confirm] backend non-OK:", res.status, txt?.slice(0, 200));
            return NextResponse.json(
                { ok: true, message: "Password has been successfully reset." },
                { status: 200 }
            );
        ...
        console.error("[reset-password-confirm] route error:", e?.message || e, { token: token ? "***" : undefined });
        return NextResponse.json(
            { ok: true, message: "Password has been successfully reset." },
            { status: 200 }
        );
