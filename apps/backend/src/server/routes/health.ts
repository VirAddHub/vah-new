...
/**
 * GET /api/healthz
 * Liveness probe for load balancers (Render, k8s): must return quickly without waiting on Postgres.
 * DB connectivity is checked on GET /api/health instead — avoid blocking deploy health checks on pool/SSL/query latency.
 */
function healthCheckMinimal(_req: Request, res: Response) {
    res.status(200).json({
        status: 'ok',
