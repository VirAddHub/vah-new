...
$pg = "/Library/PostgreSQL/17/bin/psql"
...
$cmd = "$pg -h localhost -p 5432 -U postgres -d postgres -c `"select 'ok' as ready;`""
...
$dbName = "test"
$checkDb = "$pg -h localhost -p 5432 -U postgres -tAc `"SELECT 1 FROM pg_database WHERE datname='$dbName';`" postgres"
$out = & bash -lc $checkDb
if ($out.Trim() -ne "1") {
  Write-Host "🛠  Creating DB '$dbName'..."
  $mk = "$createdb -h localhost -p 5432 -U postgres $dbName"
 & bash -lc $mk
