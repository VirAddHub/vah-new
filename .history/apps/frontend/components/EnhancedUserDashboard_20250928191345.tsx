...
export function EnhancedUserDashboard({ onLogout, onNavigate, onGoBack }: UserDashboardProps) {
    ...
    // Refetch functions
    const refetchMail = () => apiClient.get('/api/mail-items').then(r => r.ok && setMailItems(safe((r.data as any)?.items, [])));
    const refetchProfile = () => apiClient.get('/api/profile').then(r => r.ok && setProfile(safe((r.data as any)?.user, r.data)));
    const refetchSubscription = () => apiClient.get('/api/payments/subscriptions/status').then(r => r.ok && setSubscription(safe((r.data as any)?.subscription, r.data)));
    const refetchTickets = () => apiClient.get('/api/support/tickets').then(r => r.ok && setSupportTickets(safe((r.data as any)?.items, [])));
    const refetchForwarding = () => apiClient.get('/api/forwarding-requests').then(r => r.ok && setForwardingRequests(safe((r.data as any)?.items, [])));
    const refetchBilling = () => apiClient.get('/api/billing').then(r => r.ok && setBilling(safe((r.data as any)?.subscription, r.data)));
    const refetchInvoices = () => apiClient.get('/api/billing/invoices').then(r => r.ok && setInvoices(safe((r.data as any)?.items, [])));
    const refetchPlans = () => apiClient.get('/api/plans').then(r => r.ok && setPlans(safe((r.data as any)?.items, [])));
    const refetchKyc = () => apiClient.get('/api/kyc/status').then(r => r.ok && setKycStatus(r.data as any));
    ...
    const openMail = async (id: string) => {
        try {
            ...
            if (response.ok) {
                ...
                refetchMail();
                toast({ title: "Success", description: `Opened "${(response.data as any)?.description || "Mail"}".` });
            } else {
