...
import { apiClient, type MailItem, type ForwardingRequest, type User } from '@/lib/apiClient';
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        if (mailResponse.ok) {
          const data = mailResponse.data as { items?: MailItem[] };
          setMailItems(data?.items || []);
        }
        
        if (forwardingResponse.ok) {
          const data = forwardingResponse.data as { items?: ForwardingRequest[] };
          setForwardingRequests(data?.items || []);
        }
