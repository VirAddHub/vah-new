...
import { apiClient, type User, type MailItem, type ForwardingRequest } from "@/lib/apiClient";
...
export function AdminDashboard({ onLogout, onNavigate, onGoBack }: AdminDashboardProps) {
    ...
    const loadAdminData = useCallback(async () => {
        try {
            ...
            if (mailResponse.ok) {
                const mailItems = (mailResponse.data as MailItem[]) || [];
                setMailItems(mailItems);
                setStats(prev => ({ ...prev, totalMail: mailItems.length }));
            }

            if (forwardingResponse.ok) {
                const requests = (forwardingResponse.data as ForwardingRequest[]) || [];
