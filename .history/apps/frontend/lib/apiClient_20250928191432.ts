...
export const apiClient = {
    ...
    async getInvoiceLink(invoiceId: string) {
        try {
            const data = await fetchJson(`/api/billing/invoices/${invoiceId}/link`);
            return { ok: true, data };
        } catch (error: any) {
            return { ok: false, message: error.message, status: error.status };
        }
    },
    async manageSubscription(action: string) {
        try {
            const data = await fetchJson('/api/billing/subscription/manage', {
                method: 'POST',
                body: { action }
            });
            return { ok: true, data };
