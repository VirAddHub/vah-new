...
mkdir -p app/mail components/ui

# 1) Spinner (tiny, reusable)
cat > components/ui/Spinner.tsx <<'TSX'
...
# 2) /mail list page with client-side search + sort
cat > app/mail/page.tsx <<'TSX'
'use client';
...
# 3) route-level loading & error (optional if you keep inline ones)
cat > app/mail/loading.tsx <<'TSX'
...
echo "Done. Mail list UI created/updated."
